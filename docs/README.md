# Documentation Index

Central index for frontend documentation.

## Project Hub

- Root project README: [../README.md](../README.md)
- App entry and routing: [../src/main.tsx](../src/main.tsx), [../src/App.tsx](../src/App.tsx)

## Documentation Structure

- [API Integration Overview](./API_INTEGRATION.md)
- [Architecture](./architecture/overview.md)
  - [Auth Flow](./architecture/auth-flow.md)
  - [State Management](./architecture/state-management.md)
  - [Routing](./architecture/routing.md)
  - [Session & Security Model](./architecture/session-security.md)
- [API Contracts](./api/endpoints.md)
  - [Auth Contracts](./api/auth-contracts.md)
  - [Error Handling](./api/error-handling.md)
- [Guides](./guides/setup.md)
  - [CI/CD](./guides/ci-cd.md)
  - [Environment Variables](./guides/environment.md)
  - [Secrets Management](./guides/secrets-management.md)
  - [Proxy-Agnostic Design](./guides/proxy-agnostic-design.md)
  - [Development](./guides/development.md)
  - [Git Branch + Worktree Cheatsheet](./guides/git-branch-worktree-cheatsheet.md)
  - [Testing](./guides/testing.md)
  - [Docker](./guides/docker.md)
  - [Troubleshooting](./guides/troubleshooting.md)
- [Planning Index](./planning/index.md)
  - [Backlog](./planning/backlog.md)
  - [Roadmap](./planning/roadmap.md)
- [Changelog](./changelog/2026-02.md)

## Update Rule

Update this file when:

- setup changes,
- user-facing flow/routes change,
- scripts/workflow change,
- major docs are added or moved.

Last Updated: 2026-02-23
